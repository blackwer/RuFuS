...
void hot_loop(std::vector<float>& arr) {
  volatile int N;
  auto data = arr.data();
  for (int i = 0; i < N; ++i) data[i] = data[i] * 2.0f;
}
...
void hot_loop_inlining(float* arr, int N) {
  hot_loop(arr, N);
}
...
void evaluate_all_pairs(float* __restrict__ rs, float* __restrict__ rt, float* __restrict__ u, int Nsrc, int Ntrg,
                        Evaluator eval) {
  constexpr int flops = []() {
    if constexpr (requires { Evaluator::flops_per_eval; }) {
      return Evaluator::flops_per_eval;
    } else {
      return 0;
    }
  }();
...
Real eval_horner(Real* coefs, int N, Real x) {
  Real result = 0.0;

  for (int i = N - 1; i >= 0; --i) result = std::fma(result, x, coefs[i]);

  return result;
}

void evaluate_all_pairs_laplace_polynomial(float* __restrict__ rs, float* __restrict__ rt, float* __restrict__ u,
                                           int Nsrc, int Ntrg, float* __restrict__ coefs, int n_coefs) {
  evaluate_all_pairs(rs, rt, u, Nsrc, Ntrg, [coefs, n_coefs](float* __restrict__ rs, float* __restrict__ rt) {
    const float rsc = 2.0f;
    const float cen = -0.5;
    ...
    float xtmp = std::fma(r2, Rinv, cen) * rsc;
    return eval_horner<float>(coefs, n_coefs, xtmp) * Rinv;
  });
}
