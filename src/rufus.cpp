...
std::string RuFuS::Impl::create_specialized_name(const std::string& demangled_name,
  ...
  // Add const args
  for (const auto& [name, value] : const_args) oss << "_" << name << "_" << value;
...
llvm::FunctionType* RuFuS::Impl::create_specialized_function_type(llvm::Function* F,
  ...
  for (auto& Arg : F->args()) {
    if (args_to_remove.find(idx) == args_to_remove.end()) new_param_types.push_back(Arg.getType());
    idx++;
  }
...
RuFuS& RuFuS::print_module_ir() {
  if (impl->M) impl->M->print(impl->debug_out, nullptr);
  return *this;
}
...
std::uintptr_t RuFuS::compile(const std::string& demangled_name) {
  ...
  if (sym_or_err) return sym_or_err->getValue();
  ...
  if (!impl->first_compile) {
    if (auto* GV = new_module->getNamedGlobal("llvm.global_ctors")) GV->eraseFromParent();
    if (auto* GV = new_module->getNamedGlobal("llvm.global_dtors")) GV->eraseFromParent();
  } else
